...
public class SensorService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		Log.d("TAG", "Sensor Service activated");

		mClient.getTable(Sensors.class).top(1)
				.orderBy("__createdAt", QueryOrder.Descending)
				.execute(new TableQueryCallback<Sensors>() {
					...
					public void onCompleted(List<Sensors> sensors, int arg1,
						...
						if (exception != null) {
							...
							Log.d("TAG", "SensorService connection OK");
							PlantDao pDao = new PlantDao(
									getApplicationContext());
							WeatherDao wDao = new WeatherDao(
									getApplicationContext());
							TaskPlantDao tpDao = new TaskPlantDao(
									getApplicationContext());
							Sensors s = sensors.get(0);
							boolean needsWater = false;
							for (Green g : pDao.getAddedPlants()) {
								Log.d("TAG", g.getWeatherId() + " ");
								long lowHumidityLimit = wDao.getWeather(
										g.getWeatherId()).getMinHumi();
								if (s.getHumidity() < lowHumidityLimit) {
									needsWater = true;

									TaskDao td = new TaskDao(
											getApplicationContext());
									Task t = td.searchTask("Water "
											+ g.getName());
									
									// Create new object TaskPlant and
									// introduce
									// it to the database

									if (td.searchTask("Water " + g.getName()) == null) {
										t = new Task();
										t.setDescription("Water " + g.getName());
										td.addTask(t);
										t.setId(td.getLastId());

										tpDao.createTaskPlant(g, t,
	...
	private void showNotification() {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(this, 0,
				new Intent(this, ManagerActivity.class), 0);
